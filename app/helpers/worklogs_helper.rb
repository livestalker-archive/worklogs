module WorklogsHelper
  ...
  def worklogs_textilizable_wp(*args)
    ...
    text = parse_non_pre_blocks(text, obj, macros) do |text|
      [:parse_inline_attachments, :parse_wiki_links, :worklogs_parse_redmine_links_wp].each do |method_name|
  ...
  def worklogs_parse_redmine_links_wp(text, default_project, obj, attr, only_path, options)
    text.gsub!(%r{<a( [^>]+?)?>(.*?)</a>|([\s\(,\-\[\>]|^)(!)?(([a-z0-9\-_]+):)?(attachment|document|version|forum|news|message|project|commit|source|export)?(((#)|((([a-z0-9\-_]+)\|)?(r)))((\d+)((#note)?-(\d+))?)|(:)([^"\s<>][^\s<>]*?|"[^"]+?"))(?=(?=[[:punct:]][^A-Za-z0-9_/])|,|\s|\]|<|$)}) do |m|
      tag_content, leading, esc, project_prefix, project_identifier, prefix, repo_prefix, repo_identifier, sep, identifier, comment_suffix, comment_id = $1, $3, $4, $5, $6, $7, $12, $13, $10 || $14 || $20, $16 || $21, $17, $19
      if tag_content
        $&
            ...
            case prefix
              when nil
                if oid.to_s == identifier &&
                  ...
                  anchor = comment_id ? "note-#{comment_id}" : nil
                  link = link_to("##{oid}#{comment_suffix} - #{issue.subject.truncate(150)}",
            ...
            case prefix
              when nil
                if oid.to_s == identifier &&
                    issue = Issue.find_by_id(oid)
                  anchor = comment_id ? "note-#{comment_id}" : nil
                  link = link_to("##{oid}#{comment_suffix} - #{issue.subject.truncate(100)}",
